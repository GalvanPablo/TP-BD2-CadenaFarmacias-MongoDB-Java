        ...
        try (MongoClient client = MongoClients.create(MongoDBConfig.getMongoURI())) {
            ...
            List<Document> ventasPorTipo = collection.aggregate(
                Arrays.asList(
                    ...
                    // 2. Desanidar los items (1 documento por item)
                    Aggregates.unwind("$items"),
                    // 3. Agrupar por tipoProducto y sumar cantidades
                    Aggregates.group(
                        "$items.producto.tipoProducto",
                        Accumulators.sum("totalVentas", "$items.cantidad")
                    ),
                    // 4. Ordenar alfabéticamente (opcional)
                    Aggregates.sort(Sorts.ascending("_id"))
                )
            ...
            for (Document doc : ventasPorTipo) {
                String tipoProducto = doc.getString("_id");  // "_id" = tipoProducto
                int cantidad = doc.getInteger("totalVentas");
                System.out.println("- " + tipoProducto + ": " + cantidad + " unidades vendidas");
            }
        }
    }
